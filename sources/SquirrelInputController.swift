  ...
  func rimeUpdate() {
    ...
    if rimeAPI.get_context(session, &ctx) {
      ...
      if let first = rawInput.first {
        let lookupPrefixChars: [Character] = ["z", "`", "-", "=", "0", "[", "]", "\\", "'"]
        firstChar = first
      ...
      if inlineCandidate && schemaId == mySchema {
        ...
        // z:感叹号zx:问号，等。
        if firstChar == "z" && rawInput.count <= 2 {
          inlineCandidate = true
          NSApp.squirrelAppDelegate.panel?.isLookup = false
        }
        // big font
        if firstChar == "'" {
          NSApp.squirrelAppDelegate.panel?.isBigFont = true
        } else {
          NSApp.squirrelAppDelegate.panel?.isBigFont = false
        }
      }
      // dr57 end
