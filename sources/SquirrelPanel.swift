...
final class SquirrelPanel: NSPanel {
  ...
  func update(preedit: String, selRange: NSRange, caretPos: Int, candidates: [String], comments: [String], labels: [String], highlighted index: Int, page: Int, lastPage: Bool, update: Bool) {
    ...
    for i in 0..<candidates.count {
      ...
      var attrs = i == index ? theme.highlightedAttrs : theme.attrs
      // dr57 end
      ...
      if comment.isEmpty {
        ...
        noCommentCandidateFormat.replace("\u{2005}[comment]", with: "")
      }
      // 紧接着的下一行的org
      //let line = NSMutableAttributedString(string: theme.candidateFormat, attributes: labelAttrs)
      // dr57 end
      
      let line = NSMutableAttributedString(string: noCommentCandidateFormat, attributes: labelAttrs)
      ...
      // dr57 注释：候选间隔字符，我的 candidate gap 在下面，放这里效果一样。
      ...
      // candidate gap
      // dr57 注释：text是所有的文本，所有的内容。
      // dr57 注释：line一个候选项的文本。
      // gap 是加在 text.append(line) 之前，candidateRanges计算之前。
      // 这里用的不是合模型，candidate_back_color那个框是根据文字的NSRange画出来的。
      // 但就算画偏了也不影响文字的位置，它们不是合模型的“在里面”的关系。
      // 要实现stacked间隔，只能用特定字号的换行符实现。
      if (i > 0 && !vertical && theme.candidateGap != 0) {
        if linear {
          text.append(theme.horizontalSeparatorStr)
        } else {
          text.append(theme.verticalSeparatorStr)
        }
        // view.separatorWidth会扩大candidate_back_color的范围
      }
      // dr57 end
  ...
  func show() {
    ...
    if vertical {
      ...
      if (!statusMessage.isEmpty) {
        // 开关状态通知正常显示。原版效果。org
        //panelRect.origin = NSPoint(x: position.minX - theme.pagingOffset, y: position.minY - SquirrelTheme.offsetHeight - panelRect.height)
      } else if (!theme.fixedCandidateBox || isLookup) {
        // lookup时。
        // 候选框向前偏移
        // firstLabelWidth 第一个 label 的长度
        // theme.edgeInset.width 是 border_width 的长度：
        // ：edgeInset = NSMakeSize(MAX(borderWidth, cornerRadius), MAX(borderHeight, cornerRadius));
        //panelRect.origin = NSPoint(x: position.minX - theme.pagingOffset - firstLabelWidth - theme.edgeInset.width, y: position.minY - SquirrelTheme.offsetHeight - panelRect.height);
        panelRect.origin.x = panelRect.origin.x - firstLabelWidth - theme.edgeInset.width
      } else if (theme.fixedCandidateBox) {
        // dr57: fixed candidates location
        panelRect.origin = NSPoint(x: (screenRect.origin.x + screenRect.size.width * theme.fixedCandidateBoxXPercentage), y: (screenRect.origin.y + screenRect.size.height * theme.fixedCandidateBoxYPercentage))
      }
      // dr57 end
    }
