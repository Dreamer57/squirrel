...
final class SquirrelTheme {
  ...
  // dr57 begin
  // 美化候选框
  private(set) var marginX: CGFloat = 0
  private(set) var marginY: CGFloat = 0
  private(set) var paddingX: CGFloat = 0
  private(set) var paddingY: CGFloat = 0
  private(set) var candidateGap: CGFloat = 0
  // 固定候选框
  private(set) var fixedCandidateBox: Bool = false
  private(set) var fixedCandidateBoxXPercentage: CGFloat = 0
  private(set) var fixedCandidateBoxYPercentage: CGFloat = 0
  // big font mode
  private var bigFonts: [NSFont] = []
  ...
  // dr57 begin
  // candidateGap
  private(set) lazy var horizontalSeparatorStr: NSAttributedString = {
    let horizontalSeparatorAttrs: [NSAttributedString.Key: Any] = [
      .font: NSFont(name: "Helvetica", size: 4) ?? NSFont.systemFont(ofSize: 4)
    ]
    return NSAttributedString(string: String(repeating: "\u{2003}", count: Int(candidateGap)), attributes: horizontalSeparatorAttrs)
  }()
  private(set) lazy var verticalSeparatorStr: NSAttributedString = {
    let verticalSeparatorAttrs: [NSAttributedString.Key: Any] = [
      .font: NSFont(name: "Helvetica", size: 0.1) ?? NSFont.systemFont(ofSize: 0.1), // 近乎零大小
      .paragraphStyle: {
          let style = NSMutableParagraphStyle()
        style.minimumLineHeight = candidateGap // 强制行高 pt
          return style
      }()
    ]
    return NSAttributedString(string: "\n", attributes: verticalSeparatorAttrs)
  }()
  // big font
  private(set) lazy var bigAttrs: [NSAttributedString.Key: Any] = [
    .foregroundColor: candidateTextColor,
    .font: combineFonts(bigFonts, size: bigFontSize) ?? defaultFont,
    .baselineOffset: baseOffset
  ]
  private(set) lazy var bigHighlightedAttrs: [NSAttributedString.Key: Any] = [
    .foregroundColor: highlightedCandidateTextColor,
    .font: combineFonts(bigFonts, size: bigFontSize) ?? defaultFont,
    .baselineOffset: baseOffset
  ]
  ...
  private(set) lazy var edgeInset: NSSize = if self.vertical {
    ...
    // dr57 begin
    // padding
    if self.paddingX != 0 || self.paddingY != 0 || self.marginX != 0 || self.marginY != 0 {
      NSSize(width: borderWidth + paddingX * 2 + marginX * 2, height: borderHeight + paddingY * 2 + marginY * 2)
    } else {
      // org
  ...
  func load(config: SquirrelConfig, dark: Bool) {
    ...
    // dr57 begin
    // 这里是style:里定义的全局的
    marginX ?= config.getDouble("style/margin_x")
    marginY ?= config.getDouble("style/margin_y")
    paddingX ?= config.getDouble("style/padding_x")
    paddingY ?= config.getDouble("style/padding_y")
    candidateGap ?= config.getDouble("style/candidate_gap")
    fixedCandidateBox ?= config.getBool("style/fixed_candidate_box")
    fixedCandidateBoxXPercentage ?= config.getDouble("style/fixed_candidate_box_x_percentage")
    fixedCandidateBoxYPercentage ?= config.getDouble("style/fixed_candidate_box_y_percentage")
    var bigFontName: String?
    ...
    if let colorScheme = config.getString(colorSchemeOption) {
      if colorScheme != "native" {
        ...
        // dr57 begin
        // 这里是preset_color_schemes:方案里的
        marginX ?= config.getDouble("\(prefix)/margin_x")
        marginY ?= config.getDouble("\(prefix)/margin_y")
        paddingX ?= config.getDouble("\(prefix)/padding_x")
        paddingY ?= config.getDouble("\(prefix)/padding_y")
        candidateGap ?= config.getDouble("\(prefix)/candidate_gap")
        fixedCandidateBox ?= config.getBool("\(prefix)/fixed_candidate_box")
        fixedCandidateBoxXPercentage ?= config.getDouble("\(prefix)/fixed_candidate_box_x_percentage")
        fixedCandidateBoxYPercentage ?= config.getDouble("\(prefix)/fixed_candidate_box_y_percentage")
        bigFontName ?= config.getString("\(prefix)/big_font_face")
        bigFontSize ?= config.getDouble("\(prefix)/big_font_point")
        // dr57 end
      }
    } else {
      available = false
    ...
    // dr57 begin
    // big font
    bigFonts = decodeFonts(from: bigFontName)
